...
def apply_eog_filter(data, fs):
    ...
    # Step 1: Light median filter to remove only sharp spikes
    data_median = medfilt(data, kernel_size=3)  # Keep minimal
    ...
    try:
        # Step 2: Gentler bandpass filter to preserve pulse (0.1-40 Hz)
        ...
        if len(data_median) <= required_length:
            # Use even lower order filter for shorter signals
        ...
        # Step 3: Light 60Hz notch only (preserve everything else)
        ...
        if "padlen" in str(e):
            logging.warning(f"Bandpass filter failed due to short signal length ({len(data)} samples), using light median filter only")
            return data_median
        else:
